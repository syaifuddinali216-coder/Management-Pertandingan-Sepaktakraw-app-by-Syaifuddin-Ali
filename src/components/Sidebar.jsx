...
export default function Sidebar({ currentPage, onLogout }) {
  ...
  return (
    <aside style={{
      width: 240,
      background: 'linear-gradient(180deg, #3d0409 0%, #5a0812 100%)',
      borderRight: '1px solid rgba(255,215,0,0.2)',
      display: 'flex', flexDirection: 'column', flexShrink: 0,
      boxShadow: '4px 0 20px rgba(0,0,0,0.3)',
    }}>
      {/* Brand */}
      <div style={{ padding: '28px 24px 20px', borderBottom: '1px solid rgba(255,215,0,0.15)' }}>
        <div style={{ fontSize: 10, fontFamily: 'var(--font-mono)', color: 'rgba(255,215,0,0.6)', letterSpacing: 2, textTransform: 'uppercase', marginBottom: 6 }}>
          Tournament Manager
        </div>
        <div style={{ fontSize: 26, fontFamily: 'var(--font-display)', color: '#FFD700', letterSpacing: 1, lineHeight: 1.1 }}>
          SEPAK<br />TAKRAW
        </div>
      ...
      <nav style={{ flex: 1, padding: '16px 12px' }}>
        {navItems.map(item => (
          <button key={item.id} onClick={() => navigate(item.id)} style={{
            ...
            color: currentPage === item.id ? '#FFD700' : 'rgba(255,255,255,0.6)',
            fontSize: 14, fontWeight: currentPage === item.id ? 700 : 400,
            cursor: 'pointer', textAlign: 'left', marginBottom: 4, transition: 'all 0.15s',
            fontFamily: 'var(--font-body)',
            borderLeft: currentPage === item.id ? '3px solid #FFD700' : '3px solid transparent',
          }}
            onMouseEnter={e => { if (currentPage !== item.id) { e.currentTarget.style.background = 'rgba(255,255,255,0.07)'; e.currentTarget.style.color = '#fff' } }}
            onMouseLeave={e => { if (currentPage !== item.id) { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'rgba(255,255,255,0.6)' } }}
          >
